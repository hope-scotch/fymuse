...
- **MODX picker: crest factor leads (acoustic vs synth), centroid demoted; NUX deltas capped at 1.0**. Two bug fixes against the BPM-aware matcher from the previous commit:
  1. **Cathedral piano misclassified as Pad/Warm.** User's measurements: `cen 1069 Hz · tilt -10.8 dB/oct · crest 14.5 dB · RT60 5.52 s · 108 BPM` — unambiguously piano with cathedral reverb. The previous picker's centroid-led decision tree fell through to "Pad/Choir / Warm" because `cen=1069 < 1300` and `rt60 > 1.5` fired before the Piano gate (which required `cen < 600` after the v4 recalibration). Fix: restructured `rulesPickModxKeysPerf` to lead with **crest factor** instead. Crest ≥ 13 → Piano family (Vintage if RT60 > 2.5, Acoustic if cen < 1500, Modern else). Crest 10-13 → EP / Clavi / Organ. Crest < 10 → synth / pad. BPM now only acts as a tiebreaker (slow ballad → Vintage with reverb, fast dance → Syn Lead Dance), not as a hard gate.
  2. **NUX picker dominated by tilt outliers.** User's tilt of -10.8 dB/oct is way outside every preset's tilt range (-2.5 to +3.0). The unbounded `dTilt = |p.tilt - tilt| / 2.5` term made every preset's score ~3-5 on tilt alone, degenerating the picker into "least-awful tilt wins" (always 08D Lo-Fi, the preset with the lowest tilt target). Fix: cap each delta (`dCen`, `dTilt`, `dGain`) at 1.0 so extreme outliers don't dominate — other factors decide when measurements are way out of range. After cap, the cathedral-piano case picks 01A Twinverb on NUX (best clean preset, sensible best-fit when there's no actual guitar in the song).
  3. **EP threshold nudged**: medium-crest content with cen 800-1100 Hz now reaches Keyboard/Electric Piano (was Clavi). Pop Rhodes-style content classifies correctly.
