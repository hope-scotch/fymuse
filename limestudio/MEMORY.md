...
**Roadmap #4 — MIDI clock out shipped.** `MidiClockOut` in midi_input.py: own thread sends mido `clock` at 24 ppq with absolute-deadline pacing (same _beat_sleep idea), `start`/`stop` real-time messages on transport transitions, BPM read live via callable (lambda over state.click) so tempo-map changes propagate within one pulse; errors captured, port closed with a final stop. Server: `midi_clock` instance, WS `set_midi_clock {port|null}`, `/api/midi/outports`, persisted as `midi_clock_port` in show.json (re-opened on load), `midi_clock` in _full_state. Client: "Clock out" card in the MIDI modal (port select + rescan + live status; saved-but-absent port still listed). Verified with a stubbed mido: START first, 47.94 Hz measured @120 bpm (ideal 48), pulse stdev 2.75 ms, STOP on transport stop; graceful no-mido fallbacks. Settings & setup tab documents it. Remaining for #4: Ableton Link (needs LinkKit binding). NOT verified with real hardware — needs a pedal/drum machine on the Mac.

**Help drawer gets tabs: Guide | Settings & setup.** Drawer content split into `.dpane` panes with a `.dtab` rail (lime underline on active). New Settings & setup pane documents in-app: click-to-IEMs via Outputs, backing tracks, DMX (ENTTEC/Art-Net + Patch), MIDI Learn, optional mic backend (brew portaudio + pyaudio), running from source (venv + PIP_INDEX_URL fallback), building/sharing the Mac app (build.sh --public, package-for-band.sh, right-click→Open), and ~/.limestudio data layout. `.drawer code` styling added for commands.
